...
class DepthAnythingModel(pl.LightningModule):
    ...
    def get_resnet18_channels(self,resnet_model):
        channels = []

        # Initial convolutional layer
        channels.append(resnet_model.conv1.out_channels)  # Usually 64

        # Four main stages
        channels.append(resnet_model.layer1[-1].conv2.out_channels)  # Usually 64
        channels.append(resnet_model.layer2[-1].conv2.out_channels)  # Usually 128
        channels.append(resnet_model.layer3[-1].conv2.out_channels)  # Usually 256
        channels.append(resnet_model.layer4[-1].conv2.out_channels)  # Usually 512
    ...
    def __init__(self, opts):
        ...
        self.network = pipeline(task="depth-estimation", model="depth-anything/Depth-Anything-V2-Small-hf",device='cuda')
    ...
    def forward(
        ...
        cur_image = reverse_imagenet_normalize(cur_image)
        cur_image = cur_image.detach().cpu().numpy()
        cur_images_pil = [Image.fromarray((cur_image[i].transpose(1, 2, 0)*255).astype(np.uint8)) for i in range(cur_image.shape[0])]
        pred_depth = torch.stack([self.network(cur_images_pil[i])['predicted_depth'] for i in range(len(cur_images_pil))],dim=0).to(cur_cam_T_world.device)
        pred_depth = 1. / (pred_depth + 1e-6)
        #reszie it to 192x256
        # pred_depth = F.interpolate(pred_depth, (self.run_opts.image_height//2, self.run_opts.image_width//2), mode='bilinear', align_corners=False)
        depth_outputs = {'depth_pred_s0_b1hw': pred_depth}
